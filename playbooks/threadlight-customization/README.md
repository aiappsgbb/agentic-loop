...
Customization skills all read the SPEC, so a designed project is the prerequisite. If you don't have one yet, name the design skill in your prompt first:

```text
Use the threadlight-design skill to design a {process} for a {industry} customer. Quick PoC.
```
...
```text
Use the threadlight-demo-data-factory skill to generate industry-realistic demo data and idempotent Cosmos seed/reset scripts for this pilot, from SPEC § 4, § 5, and § 11d.
```
...
```text
Use the threadlight-hitl-patterns skill to generate Teams approval cards for the seven action gates declared in my SPEC § 8.
```
...
```text
Use the threadlight-workspace-ui skill to generate one operator workspace dashboard for my pilot from SPEC § 8b.
```
...
```text
Use the threadlight-event-triggers skill to scaffold ACA-first receivers for my pilot's triggers in SPEC § 10b — idempotent, with dead-letter wiring.
```
