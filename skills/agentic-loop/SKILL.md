...
- **Grounded retrieval (Foundry IQ)** - When the agent answers over enterprise/private knowledge, ground it through a **Foundry IQ** knowledge base (Azure AI Search-backed) using **agentic retrieval** - never ad hoc Search calls that bypass Foundry IQ. Make grounding **caller-aware** (ACL-filter at query time by the caller's Entra groups/personas), **cited**, and **deterministically refused** when no authorized source supports the answer - gate refusal on a **reranker relevance floor** (`reranker_threshold`), not on string-matching the model's decline phrasing. Keyless: if the agent retrieves in code, its **runtime _instance_ managed identity** needs **Search Index Data Reader** (a `postdeploy` grant, since the identity is re-minted on publish). The full identity/refusal/ACL/validation contract lives in [`references/foundry-iq-grounding.md`](references/foundry-iq-grounding.md).
