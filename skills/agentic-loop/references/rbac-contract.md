...
| Hosted-agent **runtime _instance_ managed identity** | Azure AI Search (Foundry IQ backing service) | **Search Index Data Reader** | **Direct in-code retrieval** — the agent grounds by calling the knowledge base itself (e.g. Foundry IQ `SearchIndexKnowledgeSourceParams` / `KnowledgeAgentRetrievalClient`) with `DefaultAzureCredential`, which authenticates as the **instance** MI (not `agentIdentityId`). Without it, retrieval fails `403` and the agent silently refuses. See [`foundry-iq-grounding.md`](foundry-iq-grounding.md#retrieval-identity--the-silent-403-trap). |
...
- **Direct in-code data-plane retrieval is authorized against the runtime _instance_ MI, not `agentIdentityId`.** When the agent grounds by calling AI Search / Foundry IQ **itself** (not through a brokered `CognitiveSearch` tool connection), the call authenticates as the **instance** managed identity — grant it **Search Index Data Reader** or retrieval fails `403` and the agent silently refuses (`retrieved_count=0`). The instance identity is **re-minted on every publish/deploy**, so — like the BYOK-inference grant — make it reproducible in a **`postdeploy` hook** that reads the current *Instance Identity Principal ID* from `azd ai agent show` and (re)assigns the role idempotently. Bicep cannot pre-create it (the identity does not exist until deploy). See [`foundry-iq-grounding.md`](foundry-iq-grounding.md#retrieval-identity--the-silent-403-trap).
